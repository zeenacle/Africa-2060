...
const route = document.body.dataset.route && validRoutes.includes(document.body.dataset.route) ? document.body.dataset.route : (validRoutes.includes(hashRoute) ? hashRoute : (pathRoute && validRoutes.includes(pathRoute) ? pathRoute : ''));
function routeHref(key) { return document.body.dataset.route ? `../index.html#${key}` : `#${key}`; }
...
function renderHashRoute() {
    ...
    if (pages[next]) { renderRoute(next); setupForms(); }
    else if (['invest', 'partners', 'contact'].includes(next)) { renderActionPage(next); setupForms(); }
    window.scrollTo(0, 0);
}
