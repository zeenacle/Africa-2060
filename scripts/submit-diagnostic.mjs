...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set in environment or .env');
  process.exit(2);
}
...
async function postContact() {
  ...
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': 'diag-automation-1' }, body: JSON.stringify(body) });
...
async function querySupabase() {
  const url = SUPABASE_URL.replace(/\/+$/, '') + '/rest/v1/institutional_enquiries?select=id,created_at,request_id,message&order=created_at.desc&limit=5';
